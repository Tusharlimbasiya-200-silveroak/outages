...
LOGS_FILE = os.path.dirname(os.path.dirname(os.path.dirname(ROOT_DIR))) + '/logs/mail.log'
...
def send_email_notification_of_failure(source_name, source_url, extraction_date, error_msg):
    ...
    try:
        ...
        logger.error("Unable to send email, please find the following warning message for extraction failure details: {0}".format(error))
        logger.warning("Since email delivery failed, the failure details are: source_name: {0}, source_url: {1}, extraction_date: {2}, error_message: {3}".format(source_name, source_url, extraction_date, error_msg))